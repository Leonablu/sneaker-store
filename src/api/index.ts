...
type TProductResponse = TServerResponse<{
  products: TProduct[];
  length: number;
}>;

type TPostsResponse = TServerResponse<{
  posts: TPost[];
  total: number;
  perPage: number;
  currentPage: number;
...
export const createOrder = async (orderData: {
  ...
  try {
    ...
    console.log('Успешно создан заказ:', result);
    return result;
  } catch (error) {
...
export const updateOrder = async (orderData: {
  ...
  try {
    ...
    console.log('Успешно обновлен заказ:', result);
    return result;
  } catch (error) {
